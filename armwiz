...
def deployLibrary(projectname,library):
	...
	try:
		...
		call(['rsync','-ac','{0}/'.format(library.libSourceDir),'{0}/{1}/'.format(projectname,library.libTargetDir)])
		call(['rsync','-ac','.git/modules/{0}/'.format(library.libSourceDir),'{0}/.git/modules/{1}/'.format(projectname,library.libTargetDir)])
		...
		print('ERROR copying')
		exit()
	call('cd {0}; git add .gitmodules; git add {1}; git submodule init'.format(projectname,library.libTargetDir),shell=True)
	call("cd {0}; git commit -am 'armwiz added {1}'".format(projectname,library.name),shell=True)
...
def main():
	...
	if arguments.library != None:
		for option in arguments.library:
		#	__init__(name,gitName,gitURL)
			thisGitOptionName = libraryConfig.get(option,'option')
			thisGitName = libraryConfig.get(option,'gitName')
			thisGitURL = libraryConfig.get(option,'gitURL')
			thisLibrary = library(thisGitOptionName,thisGitName,thisGitURL)
			try:
